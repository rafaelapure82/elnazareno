...
const BuscarEstudiantes = ({
    ...
    useEffect(() => {
        ...
        setLocalFilters(prev => ({
            ...
            edadMin: filters?.edadMin || '0',
            edadMax: filters?.edadMax || '30',
    ...
    const clearFilters = useCallback(() => {
        setSearchQuery('');
        const defaults = {
            genero: 'todos', nacionalidad: 'todos', tipoSangre: 'todos',
            estado: 'todos', edadMin: '0', edadMax: '30', tieneCedula: 'todos',
            sortBy: 'apellidos', order: 'asc', tipoBusqueda: 'general'
        };
                ...
                {isExpanded && (
                    ...
                    >
                        <div className="pt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pb-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <select
                                        ...
                                        className="w-full pl-5 pr-10 py-3.5 bg-white border border-slate-100 rounded-2xl font-bold text-sm text-slate-700 outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary/20 appearance-none transition-all cursor-pointer group-hover:border-slate-200"
                                    >
                                        <option value="todos">Todas las nac.</option>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-2 flex items-center gap-2">
                                    <Droplets size={12} /> Tipo de Sangre
                                </label>
                                <div className="relative group">
                                    <select
                        ...
                        <div className="flex flex-wrap items-center justify-between mt-8 pb-4 border-t border-slate-50 pt-6 gap-4">
                            <div className="flex flex-wrap items-center gap-3">
                                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100 shadow-sm">
                                    ...
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.15em]">Por: {localFilters.sortBy}</span>
                                </div>
                                
                                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100 shadow-sm">
                                    <Fingerprint size={14} className="text-emerald-500" />
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.15em]">Cédula: {localFilters.tieneCedula === 'todos' ? 'Todas' : localFilters.tieneCedula.toUpperCase()}</span>
                                </div>
                            ...
                            <div className="bg-primary/5 px-5 py-2.5 rounded-2xl border border-primary/10">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
                                    Resultados: <span className="text-primary text-sm ml-1 font-black">{totalEstudiantes}</span>
                                </p>
                            </div>
                        </div>
                    </motion.div>
