...
const ObrerosPage = () => {
    ...
    const handleCreate = async (data) => {
        try {
            ...
            Swal.fire('Éxito', 'Obrero registrado correctamente', 'success');
        } catch (e) {
            Swal.fire('Error', e.message, 'error');
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Eliminado', 'El obrero ha sido borrado', 'success');
            } catch (e) {
                Swal.fire('Error', e.message, 'error');
            }
        }
    ...
    const handleDeleteSelected = async () => {
        const result = await Swal.fire({
            title: `¿Eliminar ${selectedIds.length} obreros?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#f43f5e'
        });
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-4">
                <div className="space-y-4">
                    <div className="flex items-center gap-3 text-primary">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Personal de Servicio</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight">
                        Personal <span className="text-primary">Obrero</span>
                    </h1>
                    <p className="text-slate-500 font-medium text-lg max-w-2xl">
                        Gestión del personal de mantenimiento y servicios generales. Seguimiento de asistencia y datos laborales.
                    </p>
                </div>
